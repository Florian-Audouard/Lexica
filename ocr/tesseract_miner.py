    ...
    for elem in array:
        if func_compare(test, elem, array) and not compare(test, elem):
            if res is None:
                ...
                if abs(elem["top"] - test["top"]) < 40 < abs(res["top"] - test["top"]):
                    res = elem
                elif abs(res["top"] - elem["top"]) < 40:
                    min_res = min(
                        abs(test["left"] - (res["left"] + res["width"])),
                        abs(res["left"] - (test["left"] + test["width"])),
                    )
                    min_elem = min(
                        abs(test["left"] - (elem["left"] + elem["width"])),
                        abs(elem["left"] - (test["left"] + test["width"])),
                    )
                    if min_elem < min_res:
                        res = elem
                elif abs(elem["top"] - test["top"]) < abs(res["top"] - test["top"]):
                    res = elem
...
def concat_box(
    ...
    while change:
        ...
        if debug:
            ...
            save_as_img(array, tmp_img, file_output_name, num_page)
            print(the_change)
...
def save_as_img(array, img, file_output_name, num_page, color=(255, 0, 0)):
...
def draw_pdf(
    array, pdf_page, num_page, img_width, img_height, color=(0, 0, 1), width=1
):
...
def hienghene(
    ...
    if len(array_boxe) > 20:
        ...
        # print(save_overlap)
        if output_type == "img":
            save_as_img(save_overlap, img, file_output_name, num_page)
...
def concat_not_bold(elem1, elem2, array):
    ...
    return (
        abs((elem1["left"] + elem1["width"]) - elem2["left"]) < 70
        or abs((elem2["left"] + elem2["width"]) - elem1["left"]) < 70
    ) and abs(elem1["top"] - elem2["top"]) < 40
...
def get_most_close_bold(array, test):
    ...
    for elem in array:
        if (
            elem["bold"]
            and not compare(elem, test)
            and (elem["top"] <= test["top"] or abs(elem["top"] - test["top"]) < 25)
            and (
                (
                    abs((elem["left"] + elem["width"]) - test["left"]) < 100
                    or abs(elem["left"] - test["left"]) < 300
                )
            )
        ):
...
def nyelay_compare(elem1, elem2):
...
def nyelay(
    ...
    array_boxe = suppr_overlap(array_boxe)
    array_boxe = array_boxe = concat_box(
        array_boxe,
        concat_bold,
        debug,
        pdf_page=img,
    ...
    triRapide(array_boxe, nyelay_compare)
